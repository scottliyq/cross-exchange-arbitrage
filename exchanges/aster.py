    ...
    async def _handle_order_update(self, order_data: Dict[str, Any]):
        ...
        try:
            ...
            if hasattr(self, 'order_update_callback') and self.order_update_callback:
                if side.lower() == self.config.get('close_order_side'):
                    order_type = "CLOSE"
                else:
                    order_type = "OPEN"
...
class AsterClient(BaseExchangeClient):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.logger = TradingLogger(exchange="aster", ticker=ticker, log_to_console=False)
    ...
    async def cancel_order(self, order_id: str) -> OrderResult:
        ...
        try:
            result = await self._make_request('DELETE', '/fapi/v1/order', {
                'symbol': self.config.get('contract_id'),
                'orderId': order_id
            })
    ...
    async def get_account_positions(self) -> Decimal:
        ...
        result = await self._make_request('GET', '/fapi/v2/positionRisk', {'symbol': self.config.get('contract_id')})

        for position in result:
            if position.get('symbol') == self.config.get('contract_id'):
        ...
        try:
            ...
            for symbol_info in result['symbols']:
                if (symbol_info.get('status') == 'TRADING' and
                    ...
                    if self.config.get('quantity', Decimal(0)) < min_quantity:
                        self.logger.log(
                            f"Order quantity is less than min quantity: "
                            f"{self.config.get('quantity')} < {min_quantity}", "ERROR"
                        )
                        raise ValueError(
                            f"Order quantity is less than min quantity: "
                            f"{self.config.get('quantity')} < {min_quantity}"
                        )

                    if self.config.get('tick_size', Decimal(0)) == 0:
