...
class GrvtClient(BaseExchangeClient):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.logger = TradingLogger(exchange="grvt", ticker=ticker, log_to_console=False)
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            if self._order_update_callback is not None:
                ...
                self.logger.log(f"Deferred subscription started for {self.config.get('contract_id')}", "INFO")
    ...
    def setup_order_update_handler(self, handler) -> None:
            ...
            try:
                ...
                if 'feed' in message:
                    ...
                    if isinstance(data, dict) and leg:
                        ...
                        if contract_id != self.config.get('contract_id'):
                            return
                        ...
                        if order_id and status:
                            ...
                            if side == self.config.get('close_order_side'):
                                order_type = "CLOSE"
                            else:
                                order_type = "OPEN"
        ...
        if self._ws_client:
            try:
                asyncio.create_task(self._subscribe_to_orders(self._order_update_callback))
                self.logger.log(f"Successfully initiated subscription to order updates for {self.config.get('contract_id')}", "INFO")
            except Exception as e:
    ...
    async def _subscribe_to_orders(self, callback):
        ...
        try:
            await self._ws_client.subscribe(
                ...
                params={"instrument": self.config.get('contract_id')}
            )
            await asyncio.sleep(0)  # Small delay like in test file
            self.logger.log(f"Successfully subscribed to order updates for {self.config.get('contract_id')}", "INFO")
        except Exception as e:
    ...
    async def get_order_price(self, direction: str) -> Decimal:
        ...
        best_bid, best_ask = await self.fetch_bbo_prices(self.config.get('contract_id'))
        if best_bid <= 0 or best_ask <= 0:
            raise ValueError("Invalid bid/ask prices")

        tick_size = self.config.get('tick_size', Decimal(0))
        if direction == 'buy':
        ...
        while True:
            ...
            if attempt % 5 == 0:
                ...
                for order in active_orders:
                    if order.side == self.config.get('direction'):
    ...
    async def _get_active_close_orders(self, contract_id: str) -> int:
        ...
        for order in active_orders:
            if order.side == self.config.get('close_order_side'):
    ...
    async def get_account_positions(self) -> Decimal:
        ...
        for position in positions:
            if position.get('instrument') == self.config.get('contract_id'):
    ...
    async def get_contract_attributes(self) -> Tuple[str, Decimal]:
        ...
        for market in markets:
            if (market.get('base') == ticker and
                ...
                if self.config.get('quantity', Decimal(0)) < min_size:
                    raise ValueError(
                        f"Order quantity is less than min quantity: {self.config.get('quantity')} < {min_size}"
                    )

                return self.config.get('contract_id'), self.config.get('tick_size')
