"""Main arbitrage trading bot for GRVT and Nado exchanges."""
import asyncio
import signal
import logging
import os
...
class GrvtNadoArb:
    """Arbitrage trading bot: makes post-only orders on GRVT, and post-only orders on Nado."""
    ...
    def __init__(self, ticker: str, order_quantity: Decimal,
        ...
        self.long_spread_history = []  # Store historical long spreads (Nado bid - GRVT bid)
        self.short_spread_history = []  # Store historical short spreads (GRVT ask - Nado ask)
    ...
    def _setup_callbacks(self):
        ...
        self.order_manager.set_callbacks(
            on_order_filled=self._handle_nado_order_filled
        )
    ...
    def update_spread_statistics(self, long_spread: Decimal = None, short_spread: Decimal = None):
        ...
        Args:
            long_spread: Spread for long GRVT (Nado bid - GRVT bid)
            short_spread: Spread for short GRVT (GRVT ask - Nado ask)
        """
    ...
    def _handle_nado_order_filled(self, order_data: dict):
        ...
        try:
            ...
            if side == "SHORT":
                ...
                if self.position_tracker:
                    self.position_tracker.update_nado_position(-Decimal(filled_amount))
            else:
                order_type = "CLOSE"
                if self.position_tracker:
                    self.position_tracker.update_nado_position(Decimal(filled_amount))
            ...
            self.logger.info(
                f"[{order_id}] [{order_type}] [Nado] [FILLED]: "
            ...
            self.order_manager.nado_order_filled = True
    ...
    def initialize_nado_client(self):
        ...
        if self.nado_client is None:
            config = Config({
                'ticker': self.ticker,
                'quantity': self.order_quantity
            })
            self.nado_client = NadoClient(config.__dict__)
    ...
    async def trading_loop(self):
        ...
        if not self.grvt_client or not self.nado_client:
        ...
        self.order_manager.set_nado_config(
            self.nado_client, self.nado_contract_id, self.nado_tick_size)

        # Configure WebSocket manager
        self.ws_manager.set_grvt_config(self.grvt_client, self.grvt_contract_id)
        self.ws_manager.set_nado_config(self.nado_client, self.nado_contract_id)
        ...
        try:
            await self.ws_manager.setup_nado_bbo_polling()
            ...
            while (not self.order_book_manager.nado_order_book_ready or 
                ...
                if time.time() - start_time > timeout:
                    ...
                    self.logger.warning(
                        ...
                        f"Aster ready: {self.order_book_manager.nado_order_book_ready}")
                    break
                await asyncio.sleep(0.5)

            if self.order_book_manager.grvt_order_book_ready and self.order_book_manager.nado_order_book_ready:
        ...
        try:
            ...
            self.position_tracker.nado_position = await self.position_tracker.get_nado_position()
            ...
            self.logger.info(
                ...
                f"Nado: {self.position_tracker.nado_position}")
        except Exception as e:
        ...
        while not self.stop_flag:
            try:
                ...
                aster_best_bid, aster_best_ask = self.order_book_manager.get_nado_bbo()
    ...
    async def _update_positions(self, force: bool = False) -> bool:
        ...
        try:
            ...
            if self.stop_flag:
                return False
            self.position_tracker.nado_position = await asyncio.wait_for(
                self.position_tracker.get_nado_position(),
                timeout=3.0
            )
    ...
    async def _execute_long_trade(self):
        ...
        self.logger.info(
            ...
            f"Aster position: {self.position_tracker.nado_position}")
        ...
        while not self.order_manager.order_execution_complete and not self.stop_flag:
            if self.order_manager.waiting_for_aster_fill:
                await self.order_manager.place_nado_order(
